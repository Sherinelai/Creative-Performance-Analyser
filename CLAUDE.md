...
- `skills/mco-creative-explainer/SKILL.md` — the MCO domain reference = the app's AI system prompt
...
## The MCO grouping contract

`MCO_GROUP_MAP_GS` (`Code.js:10`) maps `inventory_format` → MCO Inventory Group
(`phone-portrait-vast-30s` and `-60s` both → `Phone Portrait VAST`); `toMcoGroup()` falls back to
stripping the `-\d+s` duration suffix. This grouping *is* the app's mental model — the Auto-Pauser
competes creatives inside an inventory group, so every table, chart and diagnosis must group the same
way. **The same mapping currently exists in three places**: `MCO_GROUP_MAP_GS`, `DN_TO_BASE` inline in
`fetchCreativeData` (`Code.js:740`), and `invToDN()` in `Dashboard.html:396`. Collapsing that to one
source is a top re-architecture target.

## Where Claude's intelligence sits

Two backend calls, both `UrlFetchApp` → `https://api.anthropic.com/v1/messages`, key from Script
Property `CLAUDE_API_KEY`, both forced to return **only** a JSON object:

| Function | Purpose | Output shape |
|---|---|---|
| `diagnoseMcoCreative` (`Code.js:2403`) | per creative: why it is in this state | `{diagnosis, format, explanation, supporting_evidence[], suggested_actions[], confidence}` — `diagnosis` is one of 13 fixed codes |
| `summarizeFormatTrends` (`Code.js:2461`) | format-level trend narrative | `{summary, format_insights[{format, trend, insight}], recommendation}` |

The system prompt is `getSkillContent()` (`Code.js:2123`): concatenates two **Google Drive** files by
Script Property `SKILL_FILE_ID` + `KB_FILE_ID`, cached in `_skillCache`; on any failure it falls back
to `MCO_SYSTEM_PROMPT_FALLBACK` (`Code.js:2160`). That fallback is mirrored into
`skills/mco-creative-explainer/SKILL.md` — read that file to understand the domain (bidding pipeline,
Auto-Pauser thresholds, WCS, throttle, the 13 diagnosis codes, metric glossary). **Rule:** any change
to a diagnosis code, threshold or lifecycle rule must land in the skill *and* the fallback *and* the
client-side fallback below, or the three disagree.

Deterministic fallbacks in the front end reimplement the same reasoning in plain JS for when the API
is unavailable: `localMcoDiagnosis` (`Dashboard.html:1282`), `localFormatSummary` (`:1435`),
`localVideoSummary` (`:2077`), `localInteractiveSummary` (`:2118`).
...
## Known weak points — the re-architecture backlog

Observed while reading the code; confirm each before acting, and record the outcome in `LEARNINGS.d/`.

1. **Triple-maintained MCO knowledge** — Drive skill / in-source fallback / client-side JS fallbacks.
2. **Triple-maintained inventory-group mapping** (see above).
3. **Monolith** — `Code.js` is merged `Config.gs` + `Analysis.gs` + fetchers + SQL builders + AI;
   `Dashboard.html` is one file of inline CSS + JS + markup. No modules, no build step, no tests.
4. **No contract between backend and front end.** `analyzeCreativePerformance` drops fields that
   `fetchCreativeData` then patches back onto `result.creativePerf` (`Code.js:793`, comment: *"Fields
   Analysis.gs drops — patch them back in"*). A typed result shape would delete that whole block.
5. **Outdated model id** — both AI calls pin `claude-sonnet-4-20250514`. Current is Sonnet 5
   (`claude-sonnet-5`). Re-verify against the `claude-api` skill before changing, and re-check the JSON
   output still parses.
6. **Looker `CLIENT_ID` hardcoded in source** (`Code.js:40`) while the secret sits in Script
   Properties. Move the id there too and keep the repo credential-free.
...
Never commit credentials. Script Properties hold the app's secrets (`LOOKER_CLIENT_SECRET`,
`CLAUDE_API_KEY`, `SKILL_FILE_ID`, `KB_FILE_ID`) — they are not in `appscript/` and must stay out.
